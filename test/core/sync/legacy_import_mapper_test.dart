...
ServerMeasurement _p(String code, DateTime at, num value, {String source = 'LEGACY'}) {
...
void main() {
  ...
  group('LegacyImportMapper', () {
    test('agrupa por instante y arma el registro antropométrico con perímetros', () {
      ...
      expect(r.height, 162, reason: 'el servidor entrega metros; el modelo local es cm');
      expect(r.bmi, 29.9);
      expect(r.waistCm, 92);
      ...
      expect(r.isSynced, isTrue, reason: 'vino del servidor: no debe re-subirse');
    });

    test('sin talla en la atención, arrastra la última conocida y calcula BMI', () {
      final batch = LegacyImportMapper.fromServer([
      ...
      expect(r2.height, 160, reason: 'forward-fill de talla, convertida a cm');
    ...
    test('composición corporal mapea grasa/músculo%/visceral/kcal/edad', () {
      final batch = LegacyImportMapper.fromServer([
        ...
        _p('KCAL', t1, 1450),
        _p('BODY_AGE', t1, 52),
        _p('MUSCLE_PCT', t1, 24.9), // % músculo esquelético (como lo guarda el legacy)
      ...
      expect(r.muscleMassKg, isNull, reason: 'el legacy nunca guardó kg de músculo');
    });

    test('acepta el código viejo VISCERAL_FAT como nivel (backend sin V25)', () {
      final batch = LegacyImportMapper.fromServer([_p('VISCERAL_FAT', t1, 7)]);
