...
class _FakeDeviceApiClient extends DeviceApiClient {
  ...
  _FakeDeviceApiClient({this.catalog = const [], this.serverCode, this.failSet = false});
...
const _omron = MeasuringDevice(
    code: 'OMRON_HBF514C', brand: 'Omron', model: 'HBF-514C', name: 'Omron HBF-514C', deviceType: 'BIOIMPEDANCE');
const _tanita = MeasuringDevice(
    code: 'TANITA_BC601', brand: 'Tanita', model: 'BC-601', name: 'Tanita BC-601', deviceType: 'BIOIMPEDANCE');
...
void main() {
  ...
  test('sin elección previa: pide elegir y carga el catálogo de la API', () async {
    final fake = _FakeDeviceApiClient(catalog: [_omron, _tanita], serverCode: null);
  ...
  test('sin red al elegir: queda pendiente y se empuja en el siguiente refresh', () async {
    final fake = _FakeDeviceApiClient(catalog: [_omron], failSet: true);
    final provider = MeasuringDeviceProvider(client: fake);
    await provider.load();

    await provider.select(_omron);
    expect(provider.pendingSync, isTrue); // falló la subida, pero la elección quedó local
  ...
  test('reconcilia con el servidor: adopta la selección remota si existe', () async {
    final fake = _FakeDeviceApiClient(catalog: [_omron, _tanita], serverCode: 'TANITA_BC601');
    final provider = MeasuringDeviceProvider(client: fake);
